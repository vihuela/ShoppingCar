...
public class DividerDecoration extends RecyclerView.ItemDecoration {
    ...
    //间隔线左右边距
    public void setDividerOffset(float dividerOffset) {
        this.dividerOffset = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerOffset, ctx.getResources().getDisplayMetrics());
    }
    //间隔线颜色，shape文件
    public void setDivider(@DrawableRes int id){
        this.mDivider = ctx.getResources().getDrawable(id);
    }
    ...
    public void drawVertical(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            mDivider.setBounds(left + dividerOffset, top, right - dividerOffset, bottom);
            mDivider.draw(c);
        }
    }
